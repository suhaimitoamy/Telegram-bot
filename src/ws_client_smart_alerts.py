...
from src.market_reader import build_market_read, breakout_valid, market_zone
...
runtime: dict[str, Any] = {
    ...
    "intrabar_watch_state": None,
    "intrabar_watch_sent_at": None,
}
...
def reset_intraday_runtime() -> None:
    ...
    runtime["intrabar_watch_state"] = None
    runtime["intrabar_watch_sent_at"] = None
    runtime["news_state"] = {
        "last_fetch_at": None,
        "cached_events": [],
    }
...
def watch_state_from_price(price: float) -> str | None:
    levels = runtime["config"]["levels"]
    logic = runtime["config"]["logic"]
    ...
    if abs(price - upper_supply) <= near_buffer or market_zone(price, levels) == "at_upper_supply":
        return "upper_supply_zone"
    if abs(price - upper_breakout) <= near_buffer or market_zone(price, levels) == "breakout_zone":
        return "upper_breakout_watch"
    return None


def should_clear_intrabar_watch(active_state: str, price: float) -> bool:
    levels = runtime["config"]["levels"]
    logic = runtime["config"]["logic"]
    hysteresis = max(logic["near_buffer"] * 0.25, 2.0)

    if active_state == "near_lower_range":
        return price >= levels["tp_mid"] + hysteresis or price <= levels["lower_decision"] - hysteresis
    if active_state == "upper_supply_zone":
        return price <= levels["upper_supply"] - hysteresis or price >= levels["upper_breakout"] + hysteresis
    if active_state == "upper_breakout_watch":
        return price <= levels["upper_supply"] - hysteresis or price >= levels["upper_extreme"] + hysteresis
    return True


def maybe_send_intrabar_watch(price: float, now: datetime) -> None:
    candles = runtime["candles"]
    if len(candles) < 2:
        return

    active_state = runtime["intrabar_watch_state"]
    if active_state and should_clear_intrabar_watch(active_state, price):
        runtime["intrabar_watch_state"] = None
        active_state = None

    candidate = watch_state_from_price(price)
    if candidate is None:
        return
    if candidate == active_state:
        return

    cooldown = int(runtime["config"]["logic"].get("intrabar_watch_cooldown_seconds", 180))
    last_sent_at = runtime["intrabar_watch_sent_at"]
    if last_sent_at is not None and (now - last_sent_at).total_seconds() < cooldown:
        return
    ...
    if read_data["state"] != candidate:
        return

    runtime["intrabar_watch_state"] = candidate
    runtime["intrabar_watch_sent_at"] = now
    runtime["last_sent_state"] = candidate
    send_market_read(state=candidate, price=price, message=read_data["message"])
    print(f"INTRABAR WATCH -> {candidate}")
...
def handle_tick(price: float, tick_dt: datetime) -> None:
    ...
    if live_bucket is None:
        ...
        maybe_send_intrabar_watch(price, tick_dt)
...
def on_open(ws: websocket.WebSocketApp) -> None:
    config = runtime["config"]
    print("WS connected")
    send_status("Bot smart alerts aktif. Intrabar watch memakai debounce, cooldown, dan hysteresis.")
    ws.send(
        json.dumps(
            {
                "action": "subscribe",
...
def run_ws() -> None:
    ...
    while True:
        ...
        if not runtime["bootstrapped"]:
            bootstrap()
            send_status("Bootstrap candle berhasil dimuat. Bot sedang mulai koneksi websocket smart alerts.")
            maybe_send_news_events(now)
            if len(runtime["candles"]) >= 2:
                maybe_send_read(runtime["candles"][-1]["close"])
